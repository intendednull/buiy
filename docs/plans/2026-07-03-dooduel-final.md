...
- **RESOLVED 2026-07-04 — LANDED as an investigation, NO render change.** Root cause = a **headless-Chromium single-knob dpr-emulation ARTIFACT, not a `buiy_core` bug**: `logical = physical / scale_factor` is correct, and on a real device winit’s two dpr signals (`scale_factor` ← `devicePixelRatio`; `physical` ← the `DevicePixelContentBox` ResizeObserver) always agree, so `logical == CSS`. Chromium’s single-knob emulation drives them independently → false 2×-large (context `deviceScaleFactor`, what the prototype saw) or 2×-small (process `--force-device-scale-factor`); correct with both set at dsf 2/3. Full analysis: `docs/reports/2026-07-04-wasm-hidpi-investigation.md`. **Shipped (SG-only, no render code):** the scale-invariance headless proxy `crates/buiy_core/tests/layout/layout_hidpi.rs` (the M4 CI form) + the faithful consistent-dpr web gate `tools/web-smoke/hidpi-check.mjs` + the report. **F9 edits no render file → REMOVED from the serial render line** (the bullets below, and R1/R2, are superseded on that point); no byte-stability risk to F1/F3/F4a/F4b. **Gate ran: SG only** (no RG — no render path touched); the actual PR title is `test(hidpi): F9 — HiDPI/web overflow is an emulation artifact, not a bug`, not the `fix(render):` title below.
...
- **HiDPI** — the **F9 headless proxy is the per-wave CI form** (already gated in F9); the **hard acceptance gate is the real-device browser check at dsf=2/3** (no top/right clip, layout fits the viewport). Real-device IME (`ime_position`, touch-only policy) is a manual milestone gate too. **(F9 resolution, 2026-07-04):** the valid milestone form is a **real device OR `tools/web-smoke/hidpi-check.mjs` with BOTH dpr knobs set** (context `deviceScaleFactor` + process `--force-device-scale-factor`); a **single-knob `deviceScaleFactor` emulation is NOT a valid HiDPI check** — it manufactures false overflow in both directions (it is what the prototype saw and mis-read as a framework bug). The journal’s unreproduced dynamic-resize transient is flagged for this real-device gate, not F9.
...
|---|---|---|---|
| R1 | **The serial `buiy_core` render line** (headline, finding M1) — F1 → F3-fill → F4a → F4b edit the same files, cannot parallelize (**2026-07-04: F9 is OUT of this line — it landed as an investigation and edits no render file; see R2**) | sequence explicitly (Merge order); keep every change additive + byte-stable (disjoint-node-set discipline) so each rebases cleanly on the prior | **Split F4b's dashed-stipple into its own PR** if the tail drags (the pre-identified cleave, §5.f) → the 14th PR |
| R2 | **F9 HiDPI unknown depth** — a web/wgpu scale-factor bug, shallow-or-deep; **blocks the mobile v1 target** and serializes with the render line | **investigate early** (parallel); the **headless proxy de-risks before a browser**; verify at dsf=2/3 on a real device at the milestone | if deep (surface-reconfigure seam), F9 can land the **headless-proxy-green fix** and defer the dynamic-resize sub-case with a note — mobile-at-load is the hard criterion, live-resize is secondary **→ RESOLVED 2026-07-04: it was NOT a bug (a headless-Chromium single-knob dpr-emulation artifact; the seam is correct). F9 landed as an investigation — the scale-invariance proxy + `hidpi-check.mjs` + report, NO render change, OUT of the serial line; the dynamic-resize transient carries to the real-device acceptance gate** |
