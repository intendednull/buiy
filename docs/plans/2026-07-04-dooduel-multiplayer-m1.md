...
- [x] **W3.1 Model split (spec §4.1).** `Dooduel` drops `game: Game`; gains `replica: RoomReplica`, `net: NetState` (`enum NetState { Offline, Solo, Joining, Connected { .. }, Dropped { .. } }`), `chat_input: String` (moved OUT of `Game` — delete the core field + `viewing_as`/`switch_seat` and the `word_display()`/`word_slots()` shims from `game.rs` now). `Msg` delta: `+ Net(ServerEvent)`, `CreateRoom`/`SubmitJoin` become intent-sending, `- SwitchSeat`, gameplay arms (`ChooseWord`, `SubmitGuess`, `Continue`, `StartMatch`, tool strokes' commit path) **send intents instead of mutating** (spec §4.2); `Msg::Net(MatchEnded)` lifts `Screen::Podium` (not `Tick`).
- [x] **W3.2 The net pump.** `net.rs`: `NetPlugin` — a `ClientNet` resource holding `Box<dyn ClientTransport>`; system in `MvuSet::Enqueue` draining `try_recv()` → `enqueue(Msg::Net(ev))`; outbound helper the reducer's `Cmd`s call. `LocalAuthorityPlugin` (solo): owns `Session` (constructed with `SessionOpts { token_gen: seeded-deterministic, fill_bots_to: 4 }`) + the in-process pair; a system pumps `session.tick(Time::elapsed())` + routes intents/events (the same loop shape `dooduel_server` runs — spec §8).
- [x] **W3.3 View re-point.** *(EXCEPT the live lobby → **W4.5**: W3 has no server, so Create/Join show the current cosmetic `NetState::Offline` lobby; the **minimal live lobby** — live `Roster`, host-gated Start, server `room_code` — is owned by W4.5 with the `WsClientTransport`. Everything else here is done.)* Mechanical: `s.game.X` → `s.replica.X` / accessor equivalents across `view/*.rs` (the deleted `game` field makes every stale read a compile error — the compiler is the audit); word row from `replica.word_slots()` (W1.1); countdown from the monotonic re-anchored `remaining` (spec §4.3 — anchor on `Msg::Net` receipt, clamp no-upward-jump); pick overlay keyed on `!replica.word_choices.is_empty()`; the lobby's **live** form is deferred to W4.5 (above); `SwitchSeat` affordances deleted (roster chips become inert badges; the "Switch to {drawer}" waiting-overlay button is removed).
  - **W2-review carry-ins (must be handled in W3):** (a) the net pump/reducer **consumes `CanvasStrokeProgress`** as a **transient overlay** — painted immediately, wiped on ANY authoritative canvas event or on a `Roster` showing the drawer disconnected (spec §3.3/§3.5 rev-2.2/R1 — the silent-discard paths send no `CanvasUndo`; never implement to a wipe trigger that doesn't arrive). (b) The **drawer's optimistic canvas derives op ids by counting its own finalizations** (dense per-turn, reset at turn start — spec §3.5/§3.3 doc) so an incoming `CanvasUndo { removed_id }` resolves against the drawer's local ops (it is never echoed its own `CanvasOpApplied`); the drawer also **finalizes locally at `MAX_OP_POINTS`** (`done: true` + fresh `stroke_id`) so the server auto-split never fires for an honest client (R3 — otherwise a >8192-point stroke desyncs its undo units), and **finalizes before sending `Undo`** (undo-of-open unreachable, R1). (c) **Podium "Play again" in networked mode is a decision point** — M1 records `StartMatch`-from-`Final` as out of scope (spec §11); wire the button to leave + re-create, or explicitly defer it, and note the choice in the commit body. (d) **Client-side input hygiene (R4):** clamp/filter `to_pixel` samples to canvas bounds BEFORE the wire (edge-drag is a normal gesture; the authority rejects out-of-bounds batches whole), and the solo path defaults a blank player name before `connect()` (the authority now rejects empty names).
- [x] **W3.4 Tests.** (a) Existing `lib.rs` probe tests re-target: solo match flows through `LocalAuthorityPlugin` (start → pick → guess via intents → podium). (b) **`tests/canvas_e2e.rs` ported (rev-2):** install `LocalAuthorityPlugin` in its unified driver + pump frames so the match reaches Drawing through the solo path. (c) New: feed a scripted `ServerEvent` sequence into `Msg::Net`, assert the view via `BuiyProbePlugin` (guesser never shows the word pre-reveal; countdown displays; chat renders). (d) Replay: record a **solo** session, replay byte-identical (spec §3.4; the **networked** replay assertion lands in W5.3 where a real networked stream first exists — rev-2).
- [x] **W3.5 RUN THE ARTIFACT.** *(HEADLESS substitution — no interactive window from the build agent: the run was satisfied by (1) the probe-driven `solo_full_match_reaches_the_podium` test — the real `Session` self-drives a full match to the podium headlessly, (2) the `canvas_e2e` live-interaction tier — real picking + synthetic strokes reach Drawing through the solo path, and (3) regenerated per-screen capture PNGs, eyeballed for parity. The **human interactive** solo run folds into the **W4.7** manual gate.)* `cargo run -p dooduel` — play a full solo match vs 3 bots to the podium; drawing, guessing, theming, avatar editor all live. Record findings in the commit body.
- [x] **W3.6 Gate + commit series.**
...
- [x] **W4.1 Deps commit (lock + deny, own commit).** Add `ewebsock = "=0.8.0"`, `async-tungstenite = { version = "0.34", default-features = false, features = ["smol-runtime"] }`, `async-net`, `async-executor`, `async-io`, `futures-lite`, `getrandom` (rev-2: the server's entropy source for tokens + room codes — already a transitive lock node, now a direct dep of `dooduel_server` only) to `[workspace.dependencies]`; run `cargo deny check`; commit the lockfile delta alone.
- [x] **W4.2 `WsClientTransport`** (`dooduel_core`, `feature = "ws-client"`): ewebsock connect; `try_recv` maps `WsMessage::Text` → `serde_json::from_str::<ServerEvent>` (decode error ⇒ log + skip, never panic); `send` serializes intents; `status()` from ewebsock events. Unit test behind the feature: intent→text→intent round-trip through the framing fns (no live socket at this tier). **Amend spec §2.1's crate diagram in this commit** (WsClientTransport → `dooduel_core` feature `ws-client`).
- [x] **W4.3 Server skeleton.** `main.rs`: parse `--port` / `DOODUEL_ADDR` — **default 7878** (rev-2: matches the GUI's default `ws://127.0.0.1:7878`; the e2e passes `--port 0` explicitly); print `LISTENING port=<n>` to stdout (the e2e's discovery line, spec §9.5); accept loop on a static `async_executor::Executor` driven by `async_io::block_on`. `wire.rs`: per-conn read task (frame-size cap 64 KiB, intent rate cap ~30/s sustained → `Error{RateLimited}`/disconnect on abuse) forwarding decoded intents into the room channel; write task draining the conn's outbox. **The production `protocol_version` gate lives here (W2-review): the read task rejects a first-frame `Create`/`Join` whose `protocol_version != PROTOCOL_VERSION` with `Error{VersionMismatch}` + close, before the intent reaches the registry/`Session` — the W2 pump modelled this in the test harness; W4 makes it real (spec §3.1).** `registry.rs`: `Create` → **getrandom-backed** 6-char `[A-Z0-9]` code collision-checked; `Join` unknown ⇒ `Error{RoomNotFound}`; **per-IP connection/Join-attempt rate limit** (rev-2: the room-code brute-force guard, spec §6.2 — a small token-bucket per remote IP at the accept/Join layer); route to the room task; GC on all-empty + grace. `room.rs`: the actor — single owner of `Session` (constructed with `SessionOpts { token_gen: getrandom-backed, match_seed: getrandom-backed, fill_bots_to: 0 }` — the seed MUST be entropy-backed, never the deterministic default; W2-review C1/R5); loop over (intent channel, 100 ms tick timer); after each intake/tick, `drain_events()` → per-recipient conn outboxes; per-turn transcript log line (acceptance evidence, spec §1.4). **Client-inbound frames have NO 64 KiB cap** (that cap is server-inbound only): the WS client must accept MB-scale `CanvasLog`/`RoomState` frames (W2-review R2).
- [x] **W4.4 Server-side tests.** In-package: registry Create/Join/GC semantics with a fake conn channel (no socket); **the per-IP join rate limit** (burst allowed, sustained brute-force rejected — rev-2); room-actor determinism (scripted intents interleaved with ticks == same script against a bare `Session`); token re-attach over reconnect including rotation + old-conn replacement (spec §6.3). **The old-conn-replacement UX test is explicit (W2-review): a live-token `Join` while the original socket is still open must close the old connection (the old conn observes the close) and rebind the seat to the new one — the room/registry drives the `Session`'s re-attach and the wire layer's socket close; assert both the reseed on the new conn and the FIN/close on the old.** Also add a `VersionMismatch` wire-gate test (a `Create`/`Join` with a bad `protocol_version` is rejected + closed before any `Welcome`).
- [x] **W4.5 Networked GUI.** `net.rs`: `CreateRoom`/`SubmitJoin` open a `WsClientTransport` to `DOODUEL_SERVER_URL` (default `ws://127.0.0.1:7878`); `NetState` drives the Join/Lobby screens (spinner on `Joining`, error toasts on `Error` events, `Dropped` → rejoin-with-token attempt).
- [x] **W4.6 CI delta.** Add to `ci.yml`: a `dooduel-web-check` step in the web-smoke job (`cargo check --target wasm32-unknown-unknown -p dooduel_web --locked` with the job's existing RUSTFLAGS) — the **wasm-safety** gate for the client graph (bevy-freeness is W0.5's tripwire — rev-2 correction: the wasm check cannot catch a bevy leak, bevy is wasm-safe). New packages ride the existing workspace jobs automatically; verify the MSRV job locally with `RUSTUP_TOOLCHAIN=1.95 cargo check --workspace --locked` before committing.
- [x] **W4.7 RUN THE ARTIFACT.** *(HEADLESS substitution, as W3.5: the automated part is `apps/dooduel_server/tests/e2e.rs` — it spawns the real `dooduel_server` bin (port 0 + `LISTENING` parse), connects two real `WsClientTransport` clients, and drives Create → Join → StartMatch → Pick → draw → correct-guess over actual sockets + a wire-level `VersionMismatch` reject, all condition-based, ~0.14 s, no flake. The **human two-window run** below is the orchestrator's/user's gate — it needs a display.)* Terminal 1: `cargo run -p dooduel_server` (binds 7878). Terminals 2+3: two `cargo run -p dooduel` instances — create a room in one, join by code in the other, play a 2-human match (draw on one screen, watch it appear on the other; guess; podium). Record findings.
- [x] **W4.8 Gate + commit series.** SG green: `fmt --check` ✓, `clippy --workspace --all-targets -D warnings` ✓, `RUSTDOCFLAGS=-D warnings cargo doc --workspace --no-deps` ✓, `nextest run --workspace` = **2211 passed / 0 failed / 109 skipped** (the `#[ignore]` GPU lane). `cargo deny check` clean (dual-tungstenite allowed). MSRV 1.95 workspace check clean. wasm `dooduel_web` check clean.
...
- [x] **W5.1 `HeadlessClient` (lib, red-first).** Wraps `WsClientTransport` + a `RoomReplica`: `pump()` (drain events → replica), `state_report() -> String` (the honest per-seat markdown view — phase/word-display/scores/chat-tail/actions-now), `canvas_png() -> Vec<u8>` (rasterize `replica.canvas_ops` through `PaintBuffer` + `image` PNG encode), plus intent passthroughs (`pick/guess/stroke/fill/undo/clear/continue_turn/leave`). Tests: feed scripted events, assert the report redacts exactly as the replica does; canvas_png round-trips through `image::load_from_memory`. *(Landed: the fold is a **pure `ReplicaFold`** — the transport-free mirror of the GUI `apply_event` [canvas-reseeds bump, Picking-boundary clear, GuessResult guessed flag, TurnEnded full-word reveal, reconnect-token capture, transient progress wiped on any authoritative canvas event / drawer-disconnected `Roster`], so it is unit- AND §9.6-replay-testable with no socket. `HeadlessClient<T: ClientTransport>` is transport-generic so tests drive the real pump via an in-process pair. Documented deviation: no monotonic countdown interpolation — a pumped-on-demand client carries the server's last-reported `remaining`.)*
- [x] **W5.2 The MCP surface (`mcp.rs`).** Hand-rolled newline-delimited JSON-RPC 2.0 over stdio (spec §7): `initialize` (advertise ONLY `tools`), `notifications/initialized`, `tools/list` (the 11 tools with JSON-schema params), `tools/call` (dispatch → `HeadlessClient`; `get_canvas` returns a base64 image content block), `ping`. stderr = logs. `Server::handle(line) -> Option<String>` drives it — no process spawn at the unit tier; malformed/unknown JSON-RPC ⇒ an error object (id echoed; null on a parse error; notifications get no response even on error), never a panic.
- [x] **W5.3 The two-process e2e** (`apps/dooduel_server/tests/e2e.rs`, spec §9.5): spawn `env!("CARGO_BIN_EXE_dooduel_server")` with `--port 0`, parse `LISTENING port=` (now deadline-bounded), connect 4 `dooduel_mcp::HeadlessClient`s; scripted full match to podium with condition-based waits (no sleeps-as-sync, no retry reliance); assert podium + a mid-match live-token reconnect (assert `RoomState`+`CanvasLog` resync AND the displaced socket's FIN) + a turn-0 multi-batch stroke + undo (a guesser resolves it to one op, dense id 0) + **the networked replay assertion (rev-2, spec §9.6):** record one client's event stream and re-fold it into a fresh `ReplicaFold` byte-identically.
  - **W4-review carry-ins (the wire-level behaviors W4 tested only at the sync/pure tier — assert them over a REAL socket here, now that the multi-client e2e exists):** (1) **old-conn replacement FIN** — a live-token `Join` closes the original socket; assert the displaced client observes the close AND the new one reseeds. (2) **rate-limit disconnect** — a client exceeding the intent cap gets `Error{RateLimited}` + a socket close. (3) **per-IP reject** — brute-force Join attempts past the per-IP bucket are rejected over the wire. (4) **host migration over the wire** — the host drops past grace, a guest observes the `Roster{host}` change. (5) **`MatchInProgress` over the wire** — a fresh `Join` mid-match is rejected. (6) **token rotation** — a reconnect rotates the token (single-use); the old token then fails `BadToken`. Also **tighten `spawn_server`'s `read_line`** with a deadline (don't block forever if the bin never prints `LISTENING`).
- [x] **W5.4 RUN THE ARTIFACT.** *(HEADLESS substitution — no display: satisfied by (1) the full two-process e2e against the REAL binaries [`full_networked_match_to_podium`, 7/7 e2e green ~1.1 s, run repeatedly no flake], and (2) a live orchestrated session — a raw-WS "host" + one real `dooduel_mcp` **bin** driven over stdin/stdout: `initialize` → `notifications/initialized` → `tools/list` [all 11 tools] → `ping` → `join_room` → `get_state` [seated LOBBY view] → a full 2-player match to the podium where the agent both DRAWS [`list_choices`→`pick_word`→`draw_stroke`] and GUESSES [`get_state`→`get_canvas` (a real 15 KB PNG)→`guess`] over stdio. The redaction held live — the guesser's Drawing `get_state` showed `_ _ _ _ _`, never the word. The server's per-turn transcript [spec §1.4] was captured end-to-end through the podium. The **human** two/three-window GUI run folds into W6.2.)*
- [x] **W5.5 Gate + commit series.** Full SG green: `fmt --all --check` ✓, `clippy --workspace --all-targets --locked -D warnings` ✓, `RUSTDOCFLAGS=-D warnings cargo doc --workspace --no-deps --locked` ✓, `nextest run --workspace --locked -j 2` = **2249 passed / 0 failed / 109 skipped** (the `#[ignore]` GPU lane, untouched by M1), `cargo test --doc --workspace --locked` ✓. `cargo deny check` clean (the `dooduel_mcp` package node is the only Cargo.lock delta — no new external crate). Commits: `feat(dooduel_mcp)` W5.1/W5.2 + `test(dooduel_server)` W5.3.
...
- [x] **W6.1 Acceptance harness.** *(Landed: `apps/dooduel_mcp/README.md` recipe + `apps/dooduel_mcp/examples/seat_driver.py` — the stdlib-only bridge that gives each LLM seat agent the campaign-proven file protocol (commands.jsonl → state.md/canvas.png) while every action traverses the real MCP stdio → WS → Session stack. Dry-run-verified headlessly: raw-WS host created+started a room, a driver seat joined via the real bin, canvas/redacted-state flowed, a scripted guess scored through the funnel, clean quit.)* A documented fish-safe launch recipe (bash script under `apps/dooduel_mcp/examples/` or a README section): start `dooduel_server`, N agent seats (each = one `dooduel_mcp` process an LLM agent drives), the room code handshake. The server's per-turn transcript is the evidence stream.
...
- [ ] **W6.3-pre (W5-review close residuals, recorded 2026-07-05):** the follow-ups ledger MUST also carry: **(d) the ENDORSED de-dup charge** — lift a shared `RoomReplica::apply_event(&mut self, ev)` into `dooduel_core::protocol` and have BOTH the GUI `apply_event` (`apps/dooduel/src/lib.rs`) and `dooduel_mcp::ReplicaFold::apply` delegate to it (erasing the current mirror + its `remaining`/RoomState-wipe divergences *by construction*), AND lift `apply_op`/`stamp_points` into `dooduel_core::canvas` shared by the GUI raster + the MCP `canvas_png`, gated by a cross-impl pixel-identity test. **(e) the §9.6 replay assertion compares the serialized `RoomReplica` only** — the fold extras (`canvas_reseeds`/`own_ops`/`live_progress`/token) are uncompared; acceptable for the replay property (they are deterministic functions of the same stream), recorded so a future tightening is deliberate. **(f) GUI warts the fold surfaced:** the GUI drain does NOT wipe transient progress on `RoomState` (the MCP fold does), and the GUI's pre-frame drawer-progress wipe is a decision worth revisiting when the shared fold lands. **(g) agent-only-host gap:** the MCP tool surface is join-only (no `create`/`start_match` — an agent joins a human-created room and the host starts); fine for the §1.4 acceptance shape, noted for a future host-capable agent. **(h) hostile-server client clamps:** the MCP client trusts server frames (no client-side radius/coord/`CanvasLog`-growth clamps) — a defense-in-depth item for M2/M3, not M1 (the M1 server is the trusted authority).
