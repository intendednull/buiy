...
- **"Transitive cascade is one-frame stale" scope** (spec § 1.3): Phase 5 honors the one-frame lag specifically for the `when_active`/`when_inactive` style-bundle path — which Phase 5 doesn't ship (see above). Geometric cascade (Cqw-driven sizing flowing through `cq_flip_check`'s read of `tree.layout()` + same-frame re-run) is handled in-frame. The Task 10 test asserts what Phase 5 actually exercises.
```
...
| § 1.3 transitive cascade (geometric cascade in-frame; style-bundle cascade deferred) | `cq_transitive_cascade_is_one_frame_stale` | 10 |
