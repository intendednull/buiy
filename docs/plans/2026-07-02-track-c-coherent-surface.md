...
**Sub-waves:** C4a Button+Checkbox (establish pattern + §6 acceptance + idempotency+flush tests); C4b Switch+Disclosure (born-state-correct visuals; grandchild-structure assertion); C4d Popover §4.1c fix (audit all Popover Style-cospawn sites before deleting `position_popover`'s re-assert — keep it if any remain). **Gate hard + RUN the gallery** (headless-green ≠ works). **Snapshot decision:** Name-tag the new widget children in C4a to stop the recurring unnamed-`entity#N` re-bless (the C3 follow-up).
