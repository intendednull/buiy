...
Extraction reads the main world through `Extract<SystemParam>`, the read-only cross-world accessor. The system signature is a fan of `Extract<Query<…>>` plus `Extract<Res<…>>`. The query below is an **illustrative subset, not exhaustive** — it omits e.g. `UiTransform.backface_visibility` and the reserved `Filter` / `MixBlendMode` / `BackdropFilter` effect components — each added as its tier lands. (The raw `CssVisibility` / `OffscreenAuto` paint-skip inputs are deliberately NOT in the fan: the `write_paint_skip` render-prep pass resolves them subtree-scoped into the computed `ComputedPaintSkip` marker, which IS — paint-order-and-top-layer.md § 5.3/§ 5.4.) It does **not** omit `UserPreferences.forced_colors` ([color-and-forced-colors.md](color-and-forced-colors.md)): gate #11 (forced-colors) is **F-tier**, so the `Extract<Res<UserPreferences>>` accessor is in the v1 fan, not deferred. The fan binds `Option<&T>` for **every** author-set component (`Background` / `Border` / `BoxShadow` / `Opacity` / `Outline` / the effect components — and `Stacking`, which a bare `Node` lacks until the `Style` bundle inserts it) because these components are inserted **independently** ([component-model.md § 1](component-model.md)); a non-`Option` `&T` query would silently drop every `Node` missing that component (e.g. an unstyled `Node` lacking `Stacking`):
    ...
    nodes: Extract<Query<
        (
            ...
            Option<&ClipRect>,                         // pillar 4 — written by WriteClipRects (clip-and-transform.md); Option
                                                       // for the same reason as Stacking/Background: WriteClipRects
            ...
            Option<&ComputedPaintSkip>,                // paint-order § 5.3/§ 5.4 — the COMPUTED subtree paint-skip marker written by
                                                       // write_paint_skip (the single skip source; extract does not read the raw
                                                       // CssVisibility/OffscreenAuto inputs — absent => paint normally)
...
**One pipeline per sample count (the same wgpu invariant, MSAA axis).** A pipeline's `MultisampleState.count` must equal the pass attachments' `sample_count` or the first `set_pipeline` fails validation ("Render pipeline targets are incompatible with render pass"). `Msaa` is a **per-view component** in Bevy (extracted onto the view render entity; a bare `Camera2d` defaults to `Msaa::Sample4`), so the typed-primitive key carries `samples` alongside `format` (`BuiyPrimitiveKey`), as does the composite key (`CompositeKey`). The VIEW-pass variants (quad + glyph into `ViewTarget::get_color_attachment()`) are specialized **per view** in a Prepare system (`pipeline::prepare_buiy_view_pipelines`, keyed `(view format, msaa.samples())`, ids carried on the per-view `BuiyViewPipelines` component the node reads — Bevy's own per-view MSAA idiom, e.g. `prepare_skybox_pipelines`). The root-group composite draws into the same multisampled window pass, so `prepare_effect_groups` keys it on the view's samples too. The **off-screen effect-group targets stay single-sampled** (`group_target_descriptor` pins `sample_count: 1`), so the step-1 group pass and the nested composite always key `samples: 1`. Shaders are untouched — MSAA changes only the pipeline `MultisampleState` and the pass attachments. The eager `pipeline::register` baseline ids (`Rgba8UnormSrgb`, 1 sample) specialize through the same shared cache (`BuiySpecializedPipelines`), so a single-sampled default-format view dedups onto them.
...
Or<(
    ...
    Changed<ComputedPaintSkip>, Changed<Containment>,
)>
```

The last two are paint-*skip* triggers, not paint-*value* triggers, but they belong in the same Or-set: a paint-suppression flip and a `content_visibility` toggle (`Containment`) each flip whether an entity paints at all. Without them a paint-skip flip never re-extracts and the prior frame's paint goes stale — change detection is the *only* damage signal ([component-model.md § 1](component-model.md)), so a skip transition that does not touch a tracked component is invisible to extract. As landed, the suppression trigger is the **computed** `ComputedPaintSkip` marker, not the raw `CssVisibility` / `OffscreenAuto` inputs: the `write_paint_skip` render-prep pass resolves those inputs subtree-scoped into the marker (root and descendants), extract reads only the marker, and the marker's **removal** (hide→show — invisible to `Changed`) rides a dedicated `RemovedComponents<ComputedPaintSkip>` stream beside the despawn stream ([2026-06-06-render-subtree-visibility-suppression-design.md](2026-06-06-render-subtree-visibility-suppression-design.md)).
...
> **Implementation (2026-06-07).** This mechanism was *specified here but not built* in R5/R6 — the extract query was `Changed`-gated yet full-*replaced* the carrier with only the changed set, and prepare re-uploaded unconditionally, so a static node was extracted once then vanished. The damage retention is realized in [`2026-06-07-render-extract-retain-damage-design.md`](2026-06-07-render-extract-retain-damage-design.md): extract builds the **full** set from an un-gated query but overwrites the carrier only when a `Changed` probe / `RemovedComponents<ResolvedLayout>` (despawn) / `theme.is_changed()` fires, else retains; prepare gates its upload on the carrier's `is_changed()`. The per-entity *patch* (resolve only the changed entity's record) named in the caveat below stays a deferred optimization — v1 rebuilds the whole set on any change.
...
BuiySet::Layout  →  Style  →  Input  →  Animate
   →  Transform-compose (bridge)  →  { WriteClipRects, WriteEffectGroups, WritePaintSkip }
