...
- `a11y/inprocess.rs` (**LANDED**, P1c-c): the demand-pulled minimal driver — `snapshot(world, TreeView)` (canonical views → `build_tree_update` → an `accesskit_consumer::Tree`, read back **through the consumer**) + the single `perform(world, action, target, data)` act-then-observe primitive (wraps `dispatch_action_request`, auto-re-snapshots) + the thin sugar `click`/`set_value`/`focus`/`increment`/`expand` + the strict-single-match `get_by_role(world, role, name, state)` + the standalone semantic-condition `wait_for(app, cond, timeout_frames)`; structured `SemanticTree`/`SemanticNode`/`NodeState`/`StateQuery` + `TreeView`. The `ref`/off-by-one is already fixed upstream (`build_tree_update` emits `node_id_for(entity)`). `accesskit_consumer` declared on `crates/buiy_core/Cargo.toml`; `buiy_verify::a11y::semantic_tree` re-points to project from this `snapshot` (one tree path). **DEFERRED here under demand-pull (co-drive §3.2 — no gallery consumer):** `act_when_actionable` + the `HitTargetable`/`Stable` actionability gates, and `set_selection`. Per the co-drive, **no AABB-only `HitTargetable` stopgap is written** — `picking::hit_test` is smallest-AABB and stacking-UNAWARE, but the real stacking-aware `hit_test` is delivered by widget-catalog C1+C3 regardless, so if `HitTargetable` is later un-deferred it reads that, never an AABB shim (follow-up #3). See [inprocess-api.md](./inprocess-api.md), [co-drive §3](../../plans/2026-06-22-widget-catalog-agent-interface-codrive.md).
