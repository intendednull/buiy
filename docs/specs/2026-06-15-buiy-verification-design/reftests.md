...
- Code: `crates/buiy_core/tests/support/mod.rs` (capture/readback: `gpu_render_app`/`render_to_image`/`readback_rgba`/`wait_for_text_ready`), `crates/buiy_core/src/render/golden.rs` (`GoldenConfig::deterministic`/`fonts_ready`), `crates/buiy_core/src/render/shader.wgsl` + `tests/render_instance.rs` (the shared SDF + its CPU port), `crates/buiy_core/src/render/mod.rs` (`DrawData`), `crates/buiy_core/tests/render_golden_harness.rs` (re-capture discipline), `crates/buiy_core/src/layout/systems.rs` (`compose_transform`, the transform-reftest oracle), `crates/buiy_verify/src/visual.rs` (superseded RMSE).
