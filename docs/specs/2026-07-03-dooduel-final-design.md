...
**RESOLUTION (2026-07-04 — LANDED as an investigation, NO render change).** Root-caused: the reported “renders ~dpr× too large + overflows” is a **headless-Chromium single-knob dpr-emulation ARTIFACT, not a `buiy_core` bug** — the scale seam is correct. Buiy is logical-px throughout; scale enters once as `logical = physical / scale_factor`, where winit reads `scale_factor` from `window.devicePixelRatio` and `physical` from the `DevicePixelContentBox` ResizeObserver (`winit-0.30`). On a real device those two dpr signals always agree, so `logical == CSS size`; Chromium’s headless emulation drives them independently (context `deviceScaleFactor` fakes the JS dpr → false 2×-large — what the prototype saw; process `--force-device-scale-factor` → false 2×-small). Proven correct with **both** signals set consistently at dsf 2/3. Full analysis: [`docs/reports/2026-07-04-wasm-hidpi-investigation.md`](../reports/2026-07-04-wasm-hidpi-investigation.md). **Shipped (SG-only, no render code):** the scale-invariance headless proxy `crates/buiy_core/tests/layout/layout_hidpi.rs` (the M4 CI form — `ResolvedLayout` byte-identical at scale_factor 1/2/3 + fits-viewport at dsf=2) + the faithful consistent-dpr web gate `tools/web-smoke/hidpi-check.mjs` + the report. **F9 edits no render file, so it is REMOVED from the serial `buiy_core` render line** — the §6.1 / §6.2 (finding M1) claim that “F9 lands in the serial render line / edits the scale-factor seam” and this section’s “the bug is in render/layout scale-factor handling” premise are **superseded** by this note; F9 adds no byte-stability risk to F1/F3/F4a/F4b. **Acceptance carry-over (§4.4):** the HiDPI milestone must use a **real device OR the both-knobs-consistent `hidpi-check.mjs`** — a single-knob `deviceScaleFactor` emulation is NOT a valid HiDPI check (it manufactures false overflow in both directions); the journal’s unreproduced dynamic-resize transient is a **separate item flagged for the real-device gate, not F9**.
...
### §8.3 Revision-4 change log (2026-07-04 — F9 resolved as an investigation)

- **F9 HiDPI (§2.10) LANDED with NO render change.** Root cause = a headless-Chromium single-knob dpr-emulation artifact, not a core scale-factor bug (the seam is correct; `logical = physical / scale_factor` holds on any real device where `devicePixelRatio` and the device-pixel-content-box agree). Full analysis: `docs/reports/2026-07-04-wasm-hidpi-investigation.md`. Deliverables (SG-only): the M4 scale-invariance headless proxy (`crates/buiy_core/tests/layout/layout_hidpi.rs`), the faithful consistent-dpr web gate (`tools/web-smoke/hidpi-check.mjs`), and the report.
- **F9 removed from the serial `buiy_core` render line** (§6.1 / §6.2 finding M1, §2.10) — it edits no render file, so it no longer serializes with F1/F3/F4a/F4b and carries no byte-stability risk. The §2.10 “reproduce → fix the render scale-factor handling” premise is superseded by the §2.10 resolution note.
- **Acceptance (§4.4):** the HiDPI milestone requires a real device or the both-knobs `hidpi-check.mjs`; a single-knob `deviceScaleFactor` emulation is not a valid HiDPI check. The unreproduced dynamic-resize transient is flagged for the real-device gate, not F9.
