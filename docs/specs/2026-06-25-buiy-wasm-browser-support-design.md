...
Per foundation § 2.9, web is a **manual-release-gate** platform; v1 automates the **compile** dimension fully and makes the **shader-conformance / paint** dimension best-effort-in-CI / enforced-where-a-GPU-exists (consistent with manual-release-gate for the render dimension):

- **Compile gate (CI, gating).** The `web-smoke` job builds the wasm web example (`trunk build` → `wasm-bindgen`), gating that `buiy` + the example compile and bind for `wasm32` — this catches arboard/lock/feature regressions. `wasm32-unknown-unknown` is also added to `deny.toml` `graph.targets` so the web dependency graph is audited.
- **Headless-browser WebGPU smoke (best-effort).** The same job then loads the build in **headless Chrome** and — **if a WebGPU adapter is present** — asserts **zero shader/pipeline errors + a non-blank canvas**. This is the **only** check that exercises the real **Tint** compiler, so the only one that catches the D2 uniformity class (the native GPU lane can't — naga is lenient by design, *verified*; it accepts the buggy shader). **Reality:** software WebGPU is unavailable on the GPU-less hosted CI runner (Dawn supports neither lavapipe nor headless SwiftShader in practice — every flag combination yields no adapter), so on that runner the shader/paint check **skips** (exit 0) rather than failing on the environment; it **enforces** wherever a WebGPU adapter exists — dev GPU hosts (where it already caught the `gradient.wgsl` regression) and a future WebGPU-capable CI runner. **Open follow-up:** make it fully CI-enforced via the `tint` CLI (Dawn's standalone WGSL validator — no adapter needed) or a self-hosted GPU runner (`docs/plans/follow-ups.md`).
...
- **DPR / canvas sizing.** The prototype saw a physical-vs-CSS size mismatch; the `gallery_web` implementation run at 1280×820 / DPR 1 showed a **clean 1:1 mapping** (canvas physical == CSS, bevy window `scale_factor` 1, the pointer position lands in the logical node boxes — confirmed by instrumenting the backend hit-test). So the mismatch is **not** a flat bug. The `max_texture_dimension_2d` clamp remains the open item for **high-DPR / large** windows (DPR × logical can exceed the device limit), still to pin there.
- **Pointer interactivity — verified; touch caveat.** v1's "*is interactive*" claim is now **empirically confirmed** (the merged spec only assumed it). Driving `gallery_web` in headless Chrome on a real AMD RDNA2 WebGPU adapter exercised the entire input pipeline — browser → winit → bevy `CursorMoved`/`MouseButtonInput` → `PointerLocation` → the buiy `bevy_picking` backend hit-test → `Pointer<Over>`/`Pointer<Click>` → the widget — across all five gallery screens (checkbox toggle, switch, stepper, rail navigation), with correct live-state updates and zero errors. **Caveat (touch — deferred):** the picking pipeline's documented one-frame hover lag (`picking/backend.rs` § 3.3) means a press with **no prior settled hover** — a touch tap, or a cold same-frame synthetic click — can arrive before the hover map updates and miss. A real desktop mouse always hovers first, so this is invisible for v1 (desktop); it must be handled when **touch** lands (resolve the press's own-frame location). Logged in `docs/plans/follow-ups.md`. This is the same lag on native — **not** web-specific.
