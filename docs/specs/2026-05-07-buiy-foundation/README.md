...
- **Platform support staging.** ~~All platforms (Windows / macOS / Linux / Android / iOS / web) at v1, or staged?~~ **Staged** ([architecture.md § 2.9](architecture.md)). **Web portion resolved:** the web (`wasm32-unknown-unknown`) target's concrete target architecture + decisions + entry conditions are specified in [`2026-06-25-buiy-wasm-browser-support-design.md`](../2026-06-25-buiy-wasm-browser-support-design.md) — prototype-validated, **WebGPU-only for v1** (WebGL2 reach deferred to its own milestone), purely additive over native, staying manual-release-gate (with an automated WebGPU compile+paint CI smoke) per § 2.9. Android/iOS staging remains open.
...
- **Bevy WASM target policy.** ~~Bevy supports WASM; the spec lists "non-Bevy frontends" as out, but Bevy-on-WASM is an in-scope Bevy target. Web a11y waits for AccessKit's web adapter; visual / input / layout work on WASM today. Whether v1 commits to WASM as a target platform is open.~~ **Resolved (staged, not v1):** Bevy-on-WASM is an in-scope target (the "non-Bevy frontends" non-goal excludes only WASM-*without*-Bevy); its target design is [`2026-06-25-buiy-wasm-browser-support-design.md`](../2026-06-25-buiy-wasm-browser-support-design.md). **Correction to the original claim (now prototype-confirmed):** Buiy does **not** build/render for `wasm32` out of the box — the v1 (WebGPU) prerequisites are (1) cfg-gating the ungated, wasm-incompatible `arboard` dep (the sole compile blocker) and (2) a **WGSL uniformity fix** (the native GPU lane can't see that buiy's shaders are WebGPU-non-conformant, so without it nothing paints). With both, a widget paints on WebGPU. The band-≤16 repack (B2) is a **WebGL2-milestone** prerequisite (it poisons the whole WebGL2 pass), not a v1 one. Web stays manual-release-gate per § 2.9; web a11y waits on an AccessKit web adapter (or a Buiy-authored DOM/ARIA sink).
