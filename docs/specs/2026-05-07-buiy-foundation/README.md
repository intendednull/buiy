...
- **Platform support staging.** ~~All platforms (Windows / macOS / Linux / Android / iOS / web) at v1, or staged?~~ **Staged** ([architecture.md § 2.9](architecture.md)). **Web portion resolved:** the web (`wasm32-unknown-unknown`) target's concrete target architecture + decisions + entry conditions are specified in [`2026-06-25-buiy-wasm-browser-support-design.md`](../2026-06-25-buiy-wasm-browser-support-design.md) — WebGPU-first + WebGL2-fallback, purely additive over native, staying manual-release-gate until a CI-usable harness exists per § 2.9. Android/iOS staging remains open.
