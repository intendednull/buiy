...
> **C7-impl findings (landed; correct the as-written above for C2's implementer):**
> 1. **The seeded editor MUST carry the display `Text("")` carrier** (the production `buiy_widgets::text_input` shape), not `TextEditState` alone. The FontsGeneration all-buffers sweep is a `Query<SyncedText>` that REQUIRES `&Text` (`sync.rs:158`); an editor with no `Text` component is never in the sweep, so the clobber never fires and the content-survival / preedit / editor-style RED proofs would be **vacuous-green**. With `Text("")` present the sweep matches, the accessor routes to the editor-owned buffer, and `apply_authored_to_buffer` `set_text("")`s it to empty (`sync.rs:512/530`) — the real Bug-3 clobber. Verified: all three RED proofs FAIL for the right reason with the carrier (`""` vs `"Hello"`; metrics `16.0` vs `28.0`; preedit destroyed).
> 2. **The reshape arm asserts `glyph_count == expected`, NOT a `changed_frames` bump.** A `FontsGeneration` increment with NO actual font-set change re-shapes to BYTE-IDENTICAL glyphs, and the producer's change-gate correctly suppresses a republish (verified: `changed_frames` does not advance). The load-bearing property is glyph SURVIVAL (a silent-no-paint regression drops them to 0); a republish-counter assertion is wrong for a no-op bump.
> 3. **A landed `editor-style-stays-live` RED proof was added** (seed at 28px, assert it survives the bump; pre-C2 the sweep resets it to the display default 16px) — C2 un-ignores it alongside content-survival and preedit.
