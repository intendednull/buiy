...
- `CanvasOp = Stroke { id, points, color, radius } | Fill { id, seed, color }` — integer canvas coordinates throughout (`stroke_segment`/`flood_fill` are already deterministic integer ops — `dooduel_core/src/canvas.rs` since the W0 extraction — so identical op sequences produce identical pixels on every replica). *Rev-2.1: the op carries the **effective** color+radius only (eraser = PAPER color + the ×1.6 radius pre-applied per §3.5) — no tool enum or erase flag on the wire; color+radius fully determine the stamp.*
...
- **Limits (also the DoS guard):** max frame 64 KiB; `Stroke` ≤ 256 points/batch; a single logged stroke op ≤ **8192 points** (`MAX_OP_POINTS`, rev-2.2/W2-review I2 — the server auto-splits an over-long accumulated stroke so every op, and each replica's per-op rasterization cost, is bounded; note the 64 KiB frame cap is **server-inbound only** — a long turn's `CanvasLog`/`RoomState` can legitimately exceed it, so the client's inbound side must not mirror the cap [W2-review R2]); guess ≤ 128 chars; name ≤ 32; room code = 6 chars `[A-Z0-9]`; avatar PNG ≤ **32 KiB raw, base64-encoded on the wire** (~43 KiB encoded, so a max-avatar `Join` frame fits the 64 KiB frame cap — *rev-2.1: the rev-2 "64 KiB (base64)" limit was internally inconsistent with the frame cap; caught at the plan gate*); per-connection intent rate cap — a token bucket, **240 intents/s sustained (burst 480)**, `Error{RateLimited}` + disconnect on the first violation (rev-2.3/W4-review: the rev-2 illustrative "~30/s" would disconnect an honest drawer — the client emits up to **one stroke batch per rendered frame** during continuous drawing, display-refresh-bound at up to ~144/s on a 144 Hz panel, so the cap must clear that ceiling with margin; the guard still bounds a true flood, and the stroke path is additionally bounded by the ≤256-points/batch + `MAX_OP_POINTS` auto-split). *Rev-2.2 (W2-review I3): the authority enforces these — name/avatar at `connect`, guess length, and canvas point/seed bounds (`0..CANVAS_W`/`0..CANVAS_H`) + radius (`1..=2×max BRUSH_SIZE`) rejected, not clamped, so a rejected op can never diverge from the drawer's local paint.*
...
| `CanvasStrokeProgress` | transient paint only — **no log entry**. The replica treats progress as a transient overlay: **wiped on any authoritative canvas event** (`CanvasOpApplied`/`CanvasUndo`/`CanvasCleared`/`CanvasLog`) **or on a `Roster` showing the drawer disconnected** (W2-review R1 — the drawer-drop/vacate discard paths emit no canvas event, so the Roster is the wipe trigger there). The finalize `CanvasOpApplied` re-stamps the same pixels idempotently. |
...
### 3.5 Stroke framing + echo policy (rev-2; op-id + liveness pinned rev-2.2 at the W2 review)
...
- A stroke spans batches under one client-chosen `stroke_id`, with `done: true` on the final batch. The `stroke_id` is a **client batching handle only** — it groups a drawer's batches and never travels in a logged op.
- **Op ids (W2-review I5):** the server assigns each logged op a **dense per-turn id** (`0, 1, 2, …`), **at finalize (append) time** — the counter resets at each turn start and advances only on a real append. An open stroke that is cancelled (undo-of-open), dropped (clear-with-open, drawer departure per §6.3) or empty mints **no** id, so no id is ever consumed invisibly. Because allocation is deterministic append-order, the drawer (never echoed its own ops) can derive its optimistic ops' ids by counting its own finalizations, so `CanvasUndo { removed_id }` resolves against its local log. An undo removes an op from the log but does **not** rewind the counter (a later op gets a fresh id). *(M1 client note, W3/`d801a2c`: the shipped client satisfies this drawer-undo-resolution requirement more simply — via the **uniform log-triggered re-render** rather than active id-counting. During its own turn the drawer's `replica.canvas_ops` is legitimately empty (no-echo), so its optimistic pixel buffer is the truth and an incoming `CanvasUndo`/`CanvasCleared` is an idempotent no-op against the empty log; the re-render only fires for a drawer when the log actually has content — a `CanvasLog` reconnect reseed, or a post-reseed clear/undo. The dense per-turn ids remain derivable exactly as documented above; the client just doesn't need to track them.)*
- **Auto-split (W2-review I2):** if an accumulating stroke would exceed `MAX_OP_POINTS` (§3.1) the server finalizes the accumulated op and opens a fresh one under the same `stroke_id` (next id), **seeded with the split point** so the joining segment is still drawn — replay of the split ops is pixel-identical to the conceptual single stroke.
- **Liveness (W2-review I6):** each accepted `done: false` batch is relayed as a transient `CanvasStrokeProgress` to every seat **except** the drawer, so guessers watch the stroke grow before it finalizes. It carries no id and never enters the log or `CanvasLog`. **Replica rule (rev-2.2/R1):** progress is a transient overlay wiped on any authoritative canvas event or on a `Roster` showing the drawer disconnected — never rely on a `CanvasUndo` that the silent-discard paths (undo-of-open, drawer drop/vacate) do not send. **Client rule:** the drawer finalizes (`done: true`) before sending `Undo`, so undo-of-open is unreachable from an honest client.
...
- **`async-tungstenite` 0.34 with `smol-runtime`** — server WS, runtime-agnostic core on futures-io; MIT; MSRV 1.85. `async-net` is the one genuinely new lock node (async-io/futures-lite/async-executor/`async-channel` are already in the lock via bevy/zbus; the server takes direct deps on them — `async-channel` carries the actor's room-intake + per-conn outboxes, W4). *(rev-2.3/W4-review, `smol-runtime` compiled + behaved exactly as claimed — no tokio fallback needed.)*
- **`getrandom`** (server-side entropy — room codes, tokens, match seeds) — the workspace declares `0.3` (an existing lock node); note the lock now carries **three getrandom majors** (0.2 pulled transitively by tungstenite's `rand`, plus the pre-existing 0.3/0.4), which `multiple-versions = "allow"` permits — all MIT/Apache-2, no advisory.
...
- **(W4-review) The per-IP limiter map is never evicted.** `dooduel_server`'s `Registry` keeps one `TokenBucket` per remote IP for the process lifetime — a slow unbounded-memory drift under churn (never a per-room concern). *Fix shape (M3, with the public-matchmaking hardening):* evict buckets that have been full/idle past a TTL, or an LRU cap.
- **(W4-review) No hard ops-per-turn cap on the canvas log.** Each op is bounded (`MAX_OP_POINTS` + the ≤256-points/batch cap), and the 240/s intent cap bounds op *arrival* rate, but a full draw window can still accumulate a large `CanvasLog` (the reconnect/late-join reseed frame). M1 accepts it (the client's inbound side is uncapped, R2; a real turn stays well under memory). *Fix shape (M3):* a per-turn op-count (or total-points) cap that coalesces/rejects once exceeded.
