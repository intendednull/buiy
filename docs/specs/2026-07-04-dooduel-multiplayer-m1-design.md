...
- **Limits (also the DoS guard):** max frame 64 KiB; `Stroke` ≤ 256 points/batch; a single logged stroke op ≤ **8192 points** (`MAX_OP_POINTS`, rev-2.2/W2-review I2 — the server auto-splits an over-long accumulated stroke into bounded ops so `CanvasLog` never overruns the frame cap); guess ≤ 128 chars; name ≤ 32; room code = 6 chars `[A-Z0-9]`; avatar PNG ≤ **32 KiB raw, base64-encoded on the wire** (~43 KiB encoded, so a max-avatar `Join` frame fits the 64 KiB frame cap — *rev-2.1: the rev-2 "64 KiB (base64)" limit was internally inconsistent with the frame cap; caught at the plan gate*); per-connection intent rate cap (server constant, ~30 intents/s sustained) — violations get `Error` and, on repeat, disconnect. *Rev-2.2 (W2-review I3): the authority enforces these — name/avatar at `connect`, guess length, and canvas point/seed bounds (`0..CANVAS_W`/`0..CANVAS_H`) + radius (`1..=2×max BRUSH_SIZE`) rejected, not clamped, so a rejected op can never diverge from the drawer's local paint.*
...
| `CanvasStrokeProgress` | transient paint only — **no log entry** (painted immediately; the finalize `CanvasOpApplied` re-stamps the same pixels, and any undo re-rasterizes the log, wiping orphaned progress pixels) |
...
- **Liveness (W2-review I6):** each accepted `done: false` batch is relayed as a transient `CanvasStrokeProgress` to every seat **except** the drawer, so guessers watch the stroke grow before it finalizes. It carries no id, never enters the log or `CanvasLog`; the eventual `CanvasOpApplied` re-stamps the same pixels (idempotent) and any undo re-rasterizes from the log (wiping orphaned progress pixels).
