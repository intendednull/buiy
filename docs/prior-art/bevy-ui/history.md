...
Filed by **@viridia** on 2026-02-02 (per WebFetch summary; date approximate). Title: "Design of bevy_a11y is BSN-unfriendly." Core argument: the `AccessNode` component bundles all a11y properties together with **private fields exposed only via inconsistent method-style setters**, so BSN — which composes templates by *merging and patching component properties* — cannot patch a11y attributes from layered templates. Viridia: "Because of this, I can well imagine wanting to merge together multiple BSN templates, each of which has opinions about various accessibility attributes." The issue closed with PR #24308. **The general lesson** Buiy embeds (per architecture.md § 2.4 + foundation README goal 3) is: every Buiy component must be small, public-fielded, observable, and decomposed by concern. No megacomponents, no private setters. This is the lesson the orchestrator pre-amble correctly highlights.
