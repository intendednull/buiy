...
- **`FontSystem`** — Process-wide handle owning the `fontdb::Database`, the locale, and the per-font cache of harfrust `ShapePlan`s. Non-`Sync`, non-`Clone`; every shape / layout / render call requires `&mut FontSystem`. Embedders keep one per process. See [`architecture.md`](architecture.md), [`integration.md`](integration.md).
  ...
  > **Correction (text campaign T9, 2026-06-11):** non-`Clone` holds; non-`Sync` is **false** in 0.19 (docs.rs 0.19.0 lists `impl Send for FontSystem` and `impl Sync for FontSystem`). The one-per-process / `&mut FontSystem` framing stands — every shape/layout/render call takes `&mut`, so the handle serializes regardless of `Sync`. See [text verification.md § 5](../../specs/2026-06-09-buiy-text-rendering-design/verification.md#5-prior-art-errata-ledger).
...
- **`Action`** — Editor command verb set: `Motion(Motion)`, `Escape`, `Insert(char)`, `Enter`, `Backspace`, `Delete`, `Indent`, `Unindent`, `Click { x, y }`, `DoubleClick { x, y }`, `TripleClick { x, y }`, `Drag { x, y }`, `Scroll { lines }`. Embedders translate OS input → `Action`. See [`editing.md` § The Action enum](editing.md#the-action-enum).
  ...
  > **Correction (text campaign T9, 2026-06-11):** 0.19 is `Scroll { pixels: f32 }`, not `Scroll { lines }` — see the correction note in [`editing.md` § The Action enum](editing.md#the-action-enum) and [text verification.md § 5](../../specs/2026-06-09-buiy-text-rendering-design/verification.md#5-prior-art-errata-ledger).
