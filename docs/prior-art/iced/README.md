...
- **Single-architect bus factor.** Héctor Ramón ([hecrj](https://github.com/hecrj)) is the only person with commit on `iced-rs/iced`. Pattern is structurally identical to Bevy-cart and to System76-jackpot51 for cosmic-text; Iced's scale plus COSMIC backing makes it bearable for now.
