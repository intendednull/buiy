...
- **The `in` / `out` / `in-out` qualifiers are a clean property-visibility design Buiy could borrow at the component level.** BSN-friendly Buiy components already commit to "small, public-fielded, observable, decomposed"; Slint's qualifiers are the same idea expressed as a property-direction taxonomy. Borrowable for component-API conventions in the widget catalog ([`/home/user/buiy/docs/specs/2026-05-07-buiy-foundation/media-and-widgets.md`](../../specs/2026-05-07-buiy-foundation/media-and-widgets.md)).
