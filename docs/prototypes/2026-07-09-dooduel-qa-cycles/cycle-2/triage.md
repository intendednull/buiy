...
- **RESOLVED (Track C2-A adjudication, 2026-07-10) — NOT A BUG; 1-based is the intended design.** The FINAL design `docs/specs/2026-07-03-dooduel-final-design.md` §5(b) **DECISION** (lines 448–451, 559) pins `i` in **`1..=hintCount`** (⇒ **33 s / 19 s**-left at the 80s reference draw); `game.rs:452`'s `for i in 1..=hint_count` and the two `hints_reveal_on_the_*` tests already encode it (both green). The measured **100 / 57 s**-left at draw=240 **IS** the design schedule (`floor(240·0.42)` / `floor(240·0.24)`), not late. The "off-by-one" traces to a **stale QA briefing** (`briefings/seat-1-mechanic-auditor.md` §2 stated the formula 0-based, `i = 0,1…`) — **corrected** (both the mechanic briefing and `finding-template.md`'s worked example). **No code change.** Re-classifies to §Not-a-bug.
