...
/// Set a text field's value via the probe SetValue channel (spec §res-Q5): resolve the field
/// by role, `set_value` through the editor's `SelectAll` + `Insert` channel, then settle.
///
/// The framework emits `TextChanged` on any value-changing `SetValue`
/// (`buiy_core::a11y::contract::honor_text_set_value`, mirroring the keyboard path), so
/// `buiy_view`'s `route_text_input` bridge fires the field's `on_input` handler and folds the
/// new text into the MVU model exactly as a keystroke would — no driver-side re-emit needed.
/// (Pre-fix, `set_value` updated the editor + a11y tree but NOT the model, so a `set_value`'d
/// Join code read as `""` by `SubmitJoin`; the W1 gate found that framework gap and it is now
/// fixed — spec §res-Q5 rev-2.2.)
fn set_value_role(
    app: &mut App,
    role: A11yRole,
    name: Option<&str>,
...
/// This is a screen-readiness pump, NOT the value-fold: the model fold happens because the
/// framework emits `TextChanged` on a value-changing `SetValue` (see [`set_value_role`]), which
/// `route_text_input` folds into the model — no driver-side re-emit.
