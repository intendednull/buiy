...
fn glyph_alpha_instance_layout() {
    ...
    // + affine[4] = 16 B, total 84 B. repr(C) with f32 fields aligns to 4, so
    // size = 84. Lock it so a field reorder/addition is caught.
    assert_eq!(std::mem::size_of::<GlyphAlphaInstance>(), 84);
