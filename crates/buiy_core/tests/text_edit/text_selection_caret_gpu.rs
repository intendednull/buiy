...
fn capture_bidi_selection() -> Vec<u8> {
    ...
    let mut app = crate::support::gpu_render_app(SEL_W, SEL_H);
    ...
    let target = crate::support::render_to_image(&mut app, SEL_W, SEL_H);
...
fn caret_blink_fixed_clock_pair() {
    ...
    let mut app = crate::support::gpu_render_app(CARET_W, CARET_H);
    let text = spawn_blink_fixture(&mut app);

    let target = crate::support::render_to_image(&mut app, CARET_W, CARET_H);
    crate::support::spawn_capture_camera(&mut app, target.clone());
    ...
    let leaked: Vec<(u32, u32)> = (0..CARET_H)
        ...
        .filter(|&(x, y)| {
            !caret_cols.contains(&x)
                && crate::support::px(&frame_a, CARET_W, x, y)
                    != crate::support::px(&frame_b, CARET_W, x, y)
        })
        .collect();
    assert!(
        leaked.is_empty(),
...
fn caret_blink_reuploads_the_glyph_buffer_only() {
    ...
    let stats = |app: &App| {
        *crate::support::render_world_resource::<BufferUploadStats>(app).expect("BufferUploadStats")
    };
