...
fn cols_where_in(
    ...
    xs.filter(|&x| {
        ys.clone()
            .any(|y| pred(crate::support::px(pixels, w, x, y)))
    })
    .collect()
}
...
fn rows_where_in(
    ...
    ys.filter(|&y| {
        xs.clone()
            .any(|x| pred(crate::support::px(pixels, w, x, y)))
    })
    .collect()
}
...
fn capture_card() -> (Vec<u8>, Rect) {
    ...
    let mut app = crate::support::gpu_render_app(CARD_W, CARD_H);
    ...
    let target = crate::support::render_to_image(&mut app, CARD_W, CARD_H);
...
fn widget_card_text_is_deterministic_and_token_tinted() {
    ...
    assert_eq!(
        crate::support::px(&frame_a, CARD_W, backdrop.0, backdrop.1),
        [0, 0, 0, 255],
    ...
    assert_px_approx(
        crate::support::px(&frame_a, CARD_W, interior.0, interior.1),
        surface,
    ...
    let darkest = xs
        ...
        .map(|(x, y)| crate::support::px(&frame_a, CARD_W, x, y))
...
fn capture_input_state(filled: bool) -> Vec<u8> {
    let _cfg = GoldenConfig::deterministic(); // the triad gates this fixture
    let mut app = crate::support::gpu_render_app(STATE_W, STATE_H);
    ...
    let target = crate::support::render_to_image(&mut app, STATE_W, STATE_H);
    crate::support::spawn_capture_camera(&mut app, target.clone());
    crate::support::finish_and_run(&mut app, 1);
    crate::support::wait_for_text_ready(&mut app, 60);
...
fn build_themed_app(palette: &Palette) -> (App, Handle<Image>) {
    ...
    let target = crate::support::render_to_image(&mut app, THEME_W, THEME_H);
    crate::support::spawn_capture_camera(&mut app, target.clone());
    crate::support::finish_and_run(&mut app, 1);
    crate::support::wait_for_text_ready(&mut app, 60);
    (app, target)
...
fn themed_text_pair_and_swap_equivalence() {
    ...
    let swapped = crate::support::readback_rgba(&mut app, target);
...
fn capture_viewport(scale: f32) -> (Vec<u8>, u32) {
    ...
    let mut app = if scale == 1.0 {
        crate::support::gpu_render_app(VIEW_W, VIEW_H)
    } else {
        crate::support::gpu_render_app_scaled(VIEW_W, VIEW_H, scale)
    };
    ...
    let target = crate::support::render_to_image(&mut app, physical.x, physical.y);
    crate::support::spawn_capture_camera(&mut app, target.clone());
    crate::support::finish_and_run(&mut app, 1);
    crate::support::wait_for_text_ready(&mut app, 60);
    let pixels = crate::support::readback_rgba(&mut app, target);
